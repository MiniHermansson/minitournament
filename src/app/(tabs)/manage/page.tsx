...
export default function ManageTournamentPage() {
  ...
  async function handleGenerateBracket() {
    ...
    const res = await fetch("/api/tournament/bracket", { method: "POST" });
    if (res.ok) {
      const fmt = tournament?.format;
      const isGroupFormat = fmt === "ROUND_ROBIN" || fmt === "GROUP_STAGE" || fmt === "GROUP_STAGE_PLAYOFF";
      router.push(isGroupFormat ? "/groups" : "/bracket");
    } else {
      const data = await res.json();
  ...
  async function handleArchive(createNew: boolean) {
    if (!confirm("Archive this tournament? It will no longer be visible to users.")) return;
    setArchiving(true);
    ...
    <div className="max-w-3xl">
      ...
      {(tournament.status === "COMPLETED" || tournament.status === "CANCELLED") && (
        <Card className="mb-6 border-amber-500/30">
          <CardHeader>
            <CardTitle>Archive Tournament</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-muted-foreground mb-4">
              Archive this tournament to make way for a new one.
            </p>
            <div className="flex gap-2">
              <Button
                ...
                disabled={archiving}
              >
                {archiving ? "Archiving..." : "Archive & Create New"}
              </Button>
              <Button
                variant="outline"
